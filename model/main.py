...
# Helper function to fetch disease details
def helper(dis):
    # Assuming the disease dataframes have the disease name as 'Disease' and the corresponding details
    desc = description[description['Disease'] == dis]['Description'].values[0]
    pre = precautions[precautions['Disease'] == dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values[0].tolist()
    med = medications[medications['Disease'] == dis]['Medication'].tolist()
    die = diets[diets['Disease'] == dis]['Diet'].tolist()
...
# Symptom dictionary (with the expanded symptoms)
symptoms_dict = {
    ...
    'abdominal_pain': 39, 'diarrhoea': 40, 'mild_fever': 41, 'yellow_urine': 42, 'yellowing_of_eyes': 43, 'acute_liver_failure': 44,
    'fluid_overload': 45, 'swelling_of_stomach': 46, 'swelled_lymph_nodes': 47, 'malaise': 48, 'blurred_and_distorted_vision': 49,
    'phlegm': 50, 'throat_irritation': 51, 'redness_of_eyes': 52, 'sinus_pressure': 53, 'runny_nose': 54, 'congestion': 55,
    'chest_pain': 56, 'weakness_in_limbs': 57, 'fast_heart_rate': 58, 'pain_during_bowel_movements': 59, 'pain_in_anal_region': 60,
    'bloody_stool': 61, 'irritation_in_anus': 62, 'neck_pain': 63, 'dizziness': 64, 'cramps': 65, 'bruising': 66, 'obesity': 67,
    'swollen_legs': 68, 'swollen_blood_vessels': 69, 'puffy_face_and_eyes': 70, 'enlarged_thyroid': 71, 'brittle_nails': 72,
    'swollen_extremeties': 73, 'excessive_hunger': 74, 'extra_marital_contacts': 75, 'drying_and_tingling_lips': 76, 'slurred_speech': 77,
    'knee_pain': 78, 'hip_joint_pain': 79, 'muscle_weakness': 80, 'stiff_neck': 81, 'swelling_joints': 82, 'movement_stiffness': 83,
    'spinning_movements': 84, 'loss_of_balance': 85, 'unsteadiness': 86, 'weakness_of_one_body_side': 87, 'loss_of_smell': 88,
    'bladder_discomfort': 89, 'foul_smell_of_urine': 90, 'continuous_feel_of_urine': 91, 'passage_of_gases': 92, 'internal_itching': 93,
    'toxic_look_(typhos)': 94, 'depression': 95, 'irritability': 96, 'muscle_pain': 97, 'altered_sensorium': 98, 'red_spots_over_body': 99,
    ...
    'stomach_bleeding': 114, 'distention_of_abdomen': 115, 'history_of_alcohol_consumption': 116, 'fluid_overload.1': 117,
...
def extract_symptoms(input_text):
    ...
    for symptom in symptoms_dict.keys():
        if re.search(r'\b' + re.escape(symptom) + r'\b', input_text, re.IGNORECASE):
            symptoms.append(symptom)
    return symptoms

# Model Prediction function with error handling for unknown predictions
def get_predicted_value(patient_symptoms):
    ...
    # Predict disease
    predicted_key = svc.predict([input_vector])[0]
    return diseases_list.get(predicted_key, "Unknown Disease")


# Define a single route for symptom prediction
...
def predict():
    ...
    if not input_text:
        return jsonify({"error": "Please provide symptoms in your message."}), 400

    # Extract valid symptoms from natural language input
    valid_symptoms = extract_symptoms(input_text)

    if not valid_symptoms:
        return jsonify({"error": "No valid symptoms found in the input."}), 400
    ...
    if predicted_disease == "Unknown Disease":
        return jsonify({"error": "Unable to determine the disease based on the provided symptoms."}), 400
